...
if (isset($_POST['max-price']) && isset($_POST['min-price'])) {
    $cena = (float)$_POST['max-price'];
    $cenamin = (float)$_POST['min-price'];  
    echo "Otrzymane ceny z formularza - Minimalna cena: " . $cenamin . " PLN, Maksymalna cena: " . $cena . " PLN<br>";
} else {
    die("Brak ustawionych wartości cen.");
} 
...
$sql = "SELECT stacje_paliw.nazwa, adresy.miasto, adresy.ulica, adresy.numer, stacje_paliw.cena_paliwa 
        ...
        WHERE stacje_paliw.cena_paliwa BETWEEN ? AND ?";
...
// Bind both min and max prices to the SQL query
$stmt->bind_param("dd", $cenamin, $cena);
$stmt->execute();
